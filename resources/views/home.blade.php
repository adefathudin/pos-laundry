...
<body class="bg-blue-gray-50" x-data="initApp()" x-init="onLoad()">
      ...
      <div class="flex flex-col bg-blue-gray-50 h-full w-full py-4">
        ...
        <div class="h-full overflow-hidden mt-4">
          <div class="h-full overflow-y-auto px-2">
            ...
            <div x-show="filteredProducts().length" class="grid grid-cols-7 gap-6 pb-3">
              <template x-for="product in filteredProducts()" :key="product.id">
                <div role="button"
                  ...
                  <img x-bind:src="`{{ asset('assets/img/') }}/` + product.kat_id + '-' + product.jenis + '.jpg'" :alt="product.nama_produk" onerror="">
            ...
            <div class="flex-1 w-full px-4 overflow-auto">
              <template x-for="item in cart" :key="item.productId">
                <div
                  ...
                  <img x-bind:src="`{{ asset('assets/img/') }}/` + item.image" alt="" class="rounded-lg h-10 w-10 bg-white shadow mr-2">
                  <div class="flex-grow">
                  ...
                  <div class="py-1">
                    <div class="w-44 grid grid-cols-4 gap-4 ml-4">
                      ...
                      <button x-on:click="clearItem(item)" class="py-1 text-blue-gray-600 hover:text-pink-500 focus:outline-none">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 inline-block" fill="none" viewBox="0 0 24 24"
                          stroke="currentColor">
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
            ...
            <div class="mb-3 text-blue-gray-700 px-3 pt-2 pb-3 rounded-lg bg-blue-gray-50">
              <div class="flex text-lg font-semibold">
                <div class="flex-grow text-left">Pembayaran
                  <span x-show="paymentMethodSelected != ''" x-text="'(' + paymentMethodSelected + ')' "></span>
                </div>
              ...
              <div class="grid grid-cols-3 gap-2 mt-2">
                <template x-for="method in paymentMethod">
                  <button x-on:click="getPaymentMethod(method)"
                    class="bg-white rounded-lg shadow hover:shadow-lg focus:outline-none inline-block px-2 py-1 text-sm" x-text="method"></button>
                </template>
              </div>
            </div>
            <div x-show="change > 0"
            ...
            <div class="mb-3 text-blue-gray-700 px-3 pt-2 pb-3 rounded-lg bg-blue-gray-50">
              <div class="flex text-lg font-semibold">
                ...
                <div class="flex text-left">
                  <select x-ref="select" data-placeholder="cari..." class="w-80 bg-white shadow rounded-lg focus:bg-white focus:shadow-lg px-2 focus:outline-none">
                    <option></option>
                    <template x-for="customer in customers">
                      <option x-bind:value="customer.id" x-text="customer.nama + ' - ' + customer.nomor_hp"></option>
                    </template>
                  </select>
                  <button x-on:click="selectedCustomer = ''" class="text-blue-gray-300 hover:text-pink-500 focus:outline-none">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 inline-block" fill="none" viewBox="0 0 24 24"
                      stroke="currentColor">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
              ...
              <div x-show="selectedCustomer > 0">
                <div class="relative flex w-full flex-wrap items-stretch mb-3">
                  ...
                  <input type="text" x-bind:value="detailCustomer.nama" placeholder="nomor hp" class="px-3 py-3 placeholder-slate-300 text-slate-600 relative bg-white bg-white rounded text-sm border-0 shadow outline-none focus:outline-none focus:ring w-full pl-10"/>
                </div>
                <div class="relative flex w-full flex-wrap items-stretch mb-3">
                  <span class="z-10 h-full leading-snug font-normal absolute text-center text-slate-300 absolute bg-transparent rounded text-base items-center justify-center w-8 pl-3 py-3">
                    <i class="bi-telephone"></i>
                  </span>
                  <input type="text" x-bind:value="detailCustomer.nomor_hp" placeholder="nomor hp" class="px-3 py-3 placeholder-slate-300 text-slate-600 relative bg-white bg-white rounded text-sm border-0 shadow outline-none focus:outline-none focus:ring w-full pl-10"/>
                </div>
                <div class="relative flex w-full flex-wrap items-stretch mb-3">
                  <span class="z-10 h-full leading-snug font-normal absolute text-center text-slate-300 absolute bg-transparent rounded text-base items-center justify-center w-8 pl-3 py-3">
                    <i class="bi-geo-alt"></i>
                  </span>
                  <input type="text" x-bind:value="detailCustomer.alamat" placeholder="nomor hp" class="px-3 py-3 placeholder-slate-300 text-slate-600 relative bg-white bg-white rounded text-sm border-0 shadow outline-none focus:outline-none focus:ring w-full pl-10"/>
                </div>
              </div>
            </div>
      ...
      <div x-show="isShowModalReceipt" class="w-96 rounded-3xl bg-white shadow-xl overflow-hidden z-10"
        ...
        <div id="receipt-content" class="text-left w-full text-sm p-6 overflow-auto">
          ...
          <div x-show="selectedCustomer > 0">
            <hr class="my-2">
            <table class="text-xs">
              <tr><td colspan="2" class="text-center">Customer</td></tr>
              <tr>
                <td><i class="bi-person"></td><td x-text="detailCustomer.nama"></td>
              </tr>
              <tr>
                <td><i class="bi-telephone"></td><td x-text="detailCustomer.nomor_hp"></td>
              </tr>
              <tr>
                <td><i class="bi-geo-alt"></td><td x-text="detailCustomer.alamat"></td>
              </tr>
            </table>
          </div>
      ...
      <div class="w-96 rounded-3xl bg-white shadow-xl overflow-hidden z-10"
        ...
        <div id="receipt-content" class="text-left w-full text-sm p-6 overflow-auto">
          ...
          <table class="text-xs">
            <tr><td colspan="2" class="text-center">Customer</td></tr>
            <tr>
              <td><i class="bi-person"></td><td>Ade Fathudin</td>
            </tr>
            <tr>
              <td><i class="bi-telephone"></td><td>0234234</td>
            </tr>
            <tr>
              <td><i class="bi-geo-alt"></td><td>Jl. Raya Kp. Sawah No.184, RT.005/RW.002, Jatimurni, Kec. Pd. Melati, Kota Bks, Jawa Barat 17431</td>
            </tr>
          </table>
          </div>
